// import { PublicResolver, PublicResolverSynchronizer, Registry } from "../../typechain";
...
//   let resolverSynchronizer: PublicResolverSynchronizer;
...
//     registry = RegistryFactory.attach(_registry.address);

//     const LayerZeroEndpointMockFactory = await ethers.getContractFactory("LayerZeroEndpointMock");
//     await LayerZeroEndpointMockFactory.deploy();

//     const PublicResolverSynchronizerFactory = await ethers.getContractFactory("PublicResolverSynchronizer");
//     const _resolverSynchronizer = await upgrades.deployProxy(PublicResolverSynchronizerFactory);
//     await _resolverSynchronizer.deployed();
//     resolverSynchronizer = PublicResolverSynchronizerFactory.attach(_resolverSynchronizer.address);

//     const PublicResolver = await ethers.getContractFactory("PublicResolver");
//     const _resolver = await upgrades.deployProxy(PublicResolver);
//     await _resolver.deployed();
//     resolver = PublicResolver.attach(_resolver.address);
//   });
...
//         expect(await registry.hasRole(await registry.DEFAULT_ADMIN_ROLE(), addr1.address), "Address 1 Have Not DEFAULT_ADMIN_ROLE.").to.equal(true);
...
//         expect(await registry.getRoleAdmin(await registry.REGISTRAR_ROLE()), "Admin ROLE is not admin of REGISTRAR_ROLE.").to.equal(await registry.ADMIN_ROLE());
//       });
//       it("ROOT_ROLE Admin is ADMIN_ROLE", async function () {
//         expect(await registry.getRoleAdmin(await registry.ROOT_ROLE()), "Admin ROLE is not admin of ROOT_ROLE.").to.equal(await registry.ADMIN_ROLE());
//       });
//     });
...
//       await registry["setRecord(bytes,bytes,address,address,uint256)"](
//         ethers.utils.toUtf8Bytes(DOMAIN),
//         ethers.utils.toUtf8Bytes(TLD),
//         addr1.address,
//         resolver.address,
//         Math.floor(expirtDate.getTime() / 1000),
//       );
//       expect(
//         await registry["exists(bytes32,bytes32)"](ethers.utils.keccak256(ethers.utils.toUtf8Bytes(DOMAIN)), ethers.utils.keccak256(ethers.utils.toUtf8Bytes(TLD))),
//         "Domain Not Exist",
