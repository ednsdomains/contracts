...
interface IAffiliateProgram {
    ...
    event IssuedReward(address indexed buyer, bytes32 fqdn, uint256 duration, address indexed rewarder, bytes32 indexed id);
    event Payout(bytes32 id, uint256 amount);
    ...
    // FUNCTIONS
    function enroll(string memory id) external;
    ...
    function issue(address buyer, bytes32 fqdn,  uint256 duration, bytes32 id) external;
    // function _payout(bytes32 id) external;
    function payout(bytes32 id) external;
    function batchPayout(bytes32[] memory ids) external;
    ...
    function setLevelMultiplierTier(uint256 index, uint256 factor, uint256 upper, uint256 lower) external;
