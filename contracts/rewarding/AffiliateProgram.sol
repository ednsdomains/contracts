...
contract AffiliateProgram is ContextUpgradeable, ERC721Upgradeable, AccessControlEnumerableUpgradeable, ERC721PausableUpgradeable, ERC721EnumerableUpgradeable, ReentrancyGuardUpgradeable, IAffiliateProgram{
    ...
    function __AffiliateProgram_init_unchained(EDNSRegistrarController _controller) internal onlyInitializing{
        ...
        _levelMultiplier = LevelMultiplier({allow: false, tiers: new LevelMultiplierTier[](0)});
    }
    ...
    modifier onlyAdminOrOperator(){
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()) || hasRole(OPERATOR_ROLE, _msgSender()), "403 - Forbidden");
        _;
    }
    ...
    modifier onlyProfileOwnerOrOperator(bytes32 id){
        require(ownerOf(uint256(id)) == _msgSender() || hasRole(OPERATOR_ROLE, _msgSender()), "403 - Forbidden");
        _;
    }
    ...
    /* ========== VIEW FUNCTIONS  ========== */
    function _getLabel(string memory id) internal virtual pure returns(bytes32){
    ...
    function available(string memory _id) public view returns(bool){
        ...
        if(_profiles[id].accepted == true || _profiles[id].rejected == false) return false;
        if(_profiles[id].accepted == false || _profiles[id].rejected == false) return false;
        if(_profiles[id].rejected == true) return true;
        return !_exists(uint256(id));
    }

    function earned(bytes32 id) public view onlyIdExist(id) onlyProfileOwnerOrOperator(id) returns(uint256){
        return _profiles[id].reward.div(365 days).mul(_rewardPerYear);
    }

    function pledged(bytes32 id) public view onlyIdExist(id) onlyProfileOwnerOrOperator(id) returns(uint256){
        return _profiles[id].pledged;
    }

    function level(bytes32 id) public view onlyIdExist(id) onlyProfileOwnerOrOperator(id) returns(uint256){
        return _profiles[id].level.div(365 days);
    }

    // Input the base reward, then output the aggregated reward. (base + pledged bonus + level bonus)
    function _multiplier(bytes32 id, uint256 base) internal view returns(uint256){
        ...
        if(allowMultiplier){
            if(_pledgeMultiplier.allow){
                ...
                uint256 _pledgeBonus = _reward.mul(_profiles[id].pledged.div(maximumPledge).mul(_pledgeMultiplier.factor)).div(100);
                _reward = _reward.add(_pledgeBonus);
            }
            if(_levelMultiplier.allow){
                for(uint i = 0; i < _levelMultiplier.tiers.length; i++){
                    if(_profiles[id].level.div(365 days) <= _levelMultiplier.tiers[i].upper && _profiles[id].level.div(365 days) >= _levelMultiplier.tiers[i].lower){
                        // Because factor is an percentage but in integer format, so it must be divide by 100 at the end
                        // _reward = (factor * _reward) / 100 + _reward
                        uint256 _levelBonus = _reward.mul(_levelMultiplier.tiers[i].factor).div(100);
    ...
    function levelMultiplierTier(uint256 index) public view returns(bytes32){
        require(index < numberOfLevelMultiplierTiers(), "500 - Index out of range");
        // upper:lower:factor
        return bytes32(abi.encodePacked(_levelMultiplier.tiers[index].upper, ":", _levelMultiplier.tiers[index].lower, ":", _levelMultiplier.tiers[index].factor));
    }
    ...
    function enroll(string memory _id) public nonReentrant whenNotPaused{
        require(available(_id), "500 - ID not available");
        bytes32 id = _getLabel(_id);
        _profiles[id]= Profile({accepted: false, rejected: false, owner: _msgSender(), pledged: 0, level: 0, reward: 0});
        emit EnrollmentRequest(_msgSender(), id);
        if(allowAutoEnroll){
    ...
    function quit(bytes32 id) public nonReentrant whenNotPaused onlyProfileOwnerOrOperator(id) onlyIdExist(id) onlyAcceptedProfile(id){
    ...
    function accept(bytes32 id) public onlyAdminOrOperator onlyIdExist(id) {
        require(_profiles[id].rejected != true, "500 - Profile has been rejected");
        _profiles[id].accepted = true;
    ...
    function _pledge(bytes32 id, uint256 amount) internal virtual nonReentrant whenNotPaused onlyIdExist(id) onlyAcceptedProfile(id){
        // require(allowPledge, "403 - Withdraw is not allow");
        require(amount > 0, "500 - Cannot pledge 0 $EDNS");
        require(_profiles[id].pledged.add(amount) <= maximumPledge, "500 - Exceed pledged limit");
        require(token.allowance(ownerOf(uint256(id)), address(this)) >= amount, "500 - Insuffient allowance");
    ...
    function _withdraw(bytes32 id, uint256 amount) internal virtual nonReentrant whenNotPaused onlyIdExist(id) onlyAcceptedProfile(id){
    ...
    function withdraw(bytes32 id, uint256 amount) public onlyProfileOwnerOrOperator(id) {
        _withdraw(id, amount);
    }

    function _claim(bytes32 id) internal virtual whenNotPaused onlyIdExist(id) onlyAcceptedProfile(id){
    ...
    function _issue(address buyer, bytes32 fqdn,  uint256 duration, bytes32 id) internal virtual onlyIdExist(id) onlyAcceptedProfile(id){
        require(_allowRewarding, "500 - Rewarding is not allow");
        require(duration > 0, "500 - Cannot issue 0 reward");
        if(_profiles[id].pledged >= minimumPledge){
            _profiles[id].reward = _profiles[id].reward.add(duration);
            emit IssuedReward(buyer, fqdn, duration, ownerOf(uint256(id)), id);
            if(allowAutoPayout){
                _payout(id);
            }
        }
    ...
    function issue(address buyer, bytes32 fqdn,  uint256 duration, bytes32 id) external onlyController{
        _issue(buyer, fqdn, duration, id);
    }

    function _payout(bytes32 id) internal virtual whenNotPaused onlyIdExist(id) onlyAcceptedProfile(id){
        require(_profiles[id].reward >= 365 days, "500 - Threshold not reach");
        uint256 _reward = _profiles[id].reward.div(365 days).mul(_rewardPerYear);
        if(allowMultiplier){
            _reward = _multiplier(id, _reward);
        }
        token.transfer(ownerOf(uint256(id)), _reward);
        emit Payout(id, _reward);
    }

    function payout(bytes32 id) public onlyAdminOrOperator{
        _payout(id);
    }

    function batchPayout(bytes32[] memory ids) public whenNotPaused onlyAdminOrOperator{
    ...
    function setPledgeMultiplierFactor(uint256 factor) public onlyAdminOrOperator{
    ...
    function setLevelMultiplierTier(uint256 index, uint256 factor, uint256 upper, uint256 lower) public onlyAdminOrOperator{
        _levelMultiplier.tiers[index] = LevelMultiplierTier({ upper: upper, lower: lower, factor: factor});
    }
