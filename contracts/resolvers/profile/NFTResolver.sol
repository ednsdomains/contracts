...
contract NFTResolver is INFTResolver, BaseResolver {
  ...
  function nft(
    ...
    string memory tld,
    uint256 chainId
  ) public view returns (NFT) {
    bytes32 fqdn = keccak256(abi.encodePacked(host, ".", domain, ".", tld));
    return _nfts[fqdn][chainId];
  }

  function nft(string memory fqdn, uint256 chainId) public view returns (NFT) {
    bytes32 fqdn_ = keccak256(bytes(fqdn));
    return _nfts[fqdn_][chainId];
  }

  function nft(bytes32 fqdn, uint256 chainId) public view returns (NFT) {
    return _nfts[fqdn][chainId];
  }
