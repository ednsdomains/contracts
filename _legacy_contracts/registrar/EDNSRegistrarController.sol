...
contract EDNSRegistrarController is AccessControlEnumerableUpgradeable {
  ...
  bytes4 private constant ACCESS_CONTROL_ENUMERABLE_ID = bytes4(keccak256("getRoleMember(bytes32,uint256)") ^ keccak256("getRoleMemberCount(bytes32)"));

  event NameRegistered(string name, bytes indexed tld, bytes32 indexed label, address indexed owner, uint256 expires);
  event NameRenewed(string name, bytes indexed tld, bytes32 indexed label, uint256 expires);
  ...
  function supportsInterface(bytes4 interfaceID) public pure override(AccessControlEnumerableUpgradeable) returns (bool) {
