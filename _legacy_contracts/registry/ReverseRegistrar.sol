...
contract ReverseRegistrar is OwnableUpgradeable, Controllable {
  ...
  function __ReverseRegistrar_init_unchained(EDNS ednsAddr, NameResolver resolverAddr) internal onlyInitializing {
    ...
    ReverseRegistrar oldRegistrar = ReverseRegistrar(edns.owner(ADDR_REVERSE_NODE));
  ...
  modifier authorised(address addr) {
    require(
      addr == msg.sender || controllers[msg.sender] || edns.isApprovedForAll(addr, msg.sender) || ownsContract(addr),
