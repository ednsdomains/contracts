...
async function getStatus(signer: Wallet, network: Network) {
  ...
  if (data?.addresses.Registrar && data?.addresses["Registry.Diamond"] && data.addresses["DefaultWrapper"]) {
    ...
    if (data?.addresses.UniversalRegistrarController) {
      ...
      const classical = await getClassicalTlds(network);
      if (classical?.length && registrar) {
        classical.push("___c_fake___");
        const q = await Promise.all([...classical.map(async (tld) => await exec("Classical", tld))]);
        return q;
      }
    }
  } else {
...
async function main() {
  ...
  if (!hre.network.config.chainId) throw new Error("???");
  console.log(await getStatus(_signer, hre.network.config.chainId));
}
